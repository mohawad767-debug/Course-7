...
int colsum(int** arr, int R, int j)
{
    int sum = 0;
    for (int i = 0;i < R;i++)
    {
...
void Print_Sum_of_each_col(vector <int> &sums,int** arr, int R, int C)
{

    for (int j = 0;j < C;j++)
    {

        sums.push_back(colsum(arr, R, j));
...
void printvector(vector <int>& sums,int R)
{
    for (int i = 0;i < R;i++)
    {
...
int main()
{
    ...
    vector<int> sums;
    Print_Sum_of_each_col(sums,arr, R, C);
    printvector(sums,R);
