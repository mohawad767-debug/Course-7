...
bool Check_account(string account_num, stclient& clientx,vector<stclient> clients)
{
...
string conactenate_str(stclient client,string delimeter)
{
    string Record = "";
...
void Return_data_to_file(vector<string> Records,string filename)
{
    fstream myfile;
...
void Delete_Client_Data(vector<stclient> clients, string accountnum)
{
    vector<string> Records;

    for (stclient& client : clients)
    {
        if (client.account_number == accountnum)
        {
        ...
        {
            Records.push_back(conactenate_str(client, "#//#"));
    ...
    Return_data_to_file(Records, filename);
...
{
    stclient client;
    vector<stclient> _clients = All_Data();
    cout << "Enter account number: "; string c; cin >> c;




    if (Check_account(c, client,_clients))
    {
        cout << "\n\nClient data\n";
        print_client_Data(client);
        cout << "\n\n";
        cout << "Are you sure to delete this client data? "; char k; cin >> k;
        
        if (toupper(k) == 'Y')
        {
            Delete_Client_Data(_clients, c);
            cout << "\n\nClient deleted succefully\n";
        }
        else
        {
            cout << "\nOkay client is not deleted\n";
        }
